...
void PID_realize() {
    ...
    /*
    if(Actual_Pressure <= Set_Pressure + 0.5 && Actual_Pressure >= Set_Pressure - 0.5){
       index = 1.0;
        PIDFreq.integral += PIDFreq.err;
    } else if (Actual_Pressure >= Set_Pressure + 3.0 && Actual_Pressure <= Set_Pressure - 3.0){
        index = 0.0;
        // PIDFreq.integral += PIDFreq.err;
    } else{
        index = (float) ((3 - fabsf(Actual_Pressure - Set_Pressure)) / 2.5);
        PIDFreq.integral += PIDFreq.err;
    }
*/
